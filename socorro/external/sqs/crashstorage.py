...
#==============================================================================
class SQSCrashStorage(CrashStorageBase):
    ...
    level semantics to talk to their underlying resource.  In the SQS
    ...
    required_config.add_option(
        'sqs_class',
        default=ConnectionContext,  # we choose a pooled connection
                                    # because we need thread safe
                                    # connection behaviors
        doc='the class responsible for connecting to SQS',
    ...
    def __init__(self, config, quit_check_callback=None):
        super(SQSCrashStorage, self).__init__(
            config,
        ...
        self.sqs = config.sqs_class(config)
    ...
    def save_raw_crash(self, raw_crash, dumps, crash_id):
        try:
            ...
            self.config.logger.debug(
                'SQSCrashStorage legacy_processing key absent in crash '
                '%s', crash_id
            )
            return
        ...
        if this_crash_should_be_queued:
            self.config.logger.debug(
                'SQSCrashStorage saving crash %s', crash_id
            )
            self.transaction(self._save_raw_crash_transaction, crash_id)
        else:
            self.config.logger.debug(
                'SQSCrashStorage not saving crash %s, legacy processing '
    ...
    def new_crashes(self):
        """This generator fetches crash_ids from SQS."""

        # We've set up SQS to require acknowledgement of processing of a
        ...
        # 'ack_crash' below.  Because SQS connections are not thread safe,
        ...
        # is run to send acknowledgments back to SQS
        self._consume_acknowledgement_queue()
        conn = self.sqs.connection()
        queues = [
            self.sqs.config.priority_queue_name,
            self.sqs.config.standard_queue_name,
            self.sqs.config.reprocessing_queue_name,
            self.sqs.config.priority_queue_name,
        ]
        while True:
            for queue in queues:
    ...
    def _consume_acknowledgement_queue(self):
        ...
        try:
            while True:
                ...
                #self.config.logger.debug(
                    #'SQSCrashStorage set to acknowledge %s',
                    #crash_id_to_be_acknowledged
                #)
                try:
                    ...
                    self.config.logger.warning(
                        'SQSCrashStorage tried to acknowledge crash %s'
                    ...
                    self.config.logger.error(
                        'SQSCrashStorage unexpected failure on %s',
                        crash_id_to_be_acknowledged,
                        exc_info=True
                    )
    ...
    def _transaction_ack_crash(self, connection, acknowledgement_token):
        self.config.logger.debug(
            'SQSCrashStorage acking with delivery_tag %s',
...
#==============================================================================
class ReprocessingSQSCrashStore(SQSCrashStorage):
    SQSCrashStorage.required_config.routing_key.set_default(
        'socorro.reprocessing',
        force=True
    )
    SQSCrashStorage.required_config.filter_on_legacy_processing \
